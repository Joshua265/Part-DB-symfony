...
class AjaxUI {
  ...
  public start(disabled: boolean = false) {
    ...
    if (this.BASE.match(/https/gm) === null) {
  ...
  public registerLinks() {
    ...
    $('a')
      .not(
        ".link-external, [data-no-ajax], .page-link, [href^='javascript'], [href^='#']"
      )
      .unbind('click')
  ...
  private onAjaxError(event, request, settings) {
    ...
    var alert = bootbox.alert({
      ...
      message: function () {
        let msg =
          'Error getting data from Server! <b>Status Code: ' +
          request.status +
          '</b>';

        msg +=
          '<br><br><a class="btn btn-link" data-toggle="collapse" href="#iframe_div" >' +
          'Show response' +
          '</a>';
        msg +=
          "<div class=\" collapse\" id='iframe_div'><iframe height='512' width='100%' id='iframe'></iframe></div>";
      ...
      promise.then(function () {
        ...
        if ($table.data('part_table')) {
          ...
          $('#dt').on('select.dt deselect.dt', function (e, dt, items) {
            ...
            let selected_ids_string = selected_elements
              .data()
